...
# NOTE: Borrowed from https://raw.githubusercontent.com/state-spaces/s4/refs/heads/main/models/s4/s4.py
...
_c2r = lambda x: torch.view_as_real(x) if x.is_complex() else x
_r2c = lambda x: torch.view_as_complex(x) if (not x.is_complex()) and x.size(-1)==2 else x
...
def transition(measure, N, **measure_args):
    ...
    # Legendre (scaled)
        ...
        # NOTE: See Eq. 24 in the appendix of the HiPPO paper.
        #       S4 paper set alpha = 0.0, beta=0.0 in the equations, but this results in an invalid form.
        #       So, the official code of HiPPO uses beta=0.01 as a default value.
        beta = 0.01
        A = -np.ones((N,N))
        A = np.tril(A) + 0.5*np.eye(N)
        A = A - 0.5*beta*np.eye(N)
        inv_zeta = 1.0 * beta # = (ss.gamma(1-alpha) * beta**(alpha-1))**-1
        B = (inv_zeta**0.5)*np.ones(N)
        B = B[:,None]
    else:
...
def rank_correction(measure, N, rank=1, dtype=torch.float):
    ...
    if measure == 'legs':
        ...
        assert rank >= 1
        # NOTE: PQ^T = 0.5*np.ones((N,N)), so we can use P = Q = sqrt(0.5) * np.ones(N)
        P = (0.5*torch.ones(N)).sqrt()
        P = P.unsqueeze(0) # rank 1
...
class SSMKernel(Kernel):
    ...
    dt_transform: Transform function for parameterization of dt (default 'softplus', used to be 'exp') NOTE: 'exp' is still the default if the S4Block is instantiated w/o specific options.
    ...
    def init_dt(self):
        ...
        if self.deterministic:  # Meant for debugging
            ...
            else: # NOTE: Customized for uniform sampling in the kernel space.
                inv_dt_min = inv_transform(torch.tensor(self.dt_min), self.dt_transform)
                inv_dt_max = inv_transform(torch.tensor(self.dt_max), self.dt_transform)
                inv_dt = torch.rand(*shape, dtype=self.dtype) * (
                    inv_dt_max - inv_dt_min
                ) + inv_dt_min
            # if self.dt_transform != 'exp':
...
class SSMKernelDPLR(SSMKernelDiag):
    ...
    def _setup_C(self, L):
        ...
        self.C.copy_(_c2r(C_)) # NOTE: This raises an error when using nn.DataParallel().
    ...
    def forward(self, state=None, rate=1.0, L=None):
        ...
        r = cauchy_mult(v, z, A) # (B+1+R, C+R, H, L)
